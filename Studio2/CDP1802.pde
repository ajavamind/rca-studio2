...
void CPU_Reset()
{
  ...
  DF = DF & 1;                                                                    // Make DF a valid value as it is 1-bit.
    ...
    for (int i = 0;i < 2048;i++) studio2_4k[i] = _studio2[i];
    if (console == STUDIO3) {
      for (int i = 0; i<64; i++) {
...
static void CPU_WriteMemory(int address, int data)
{
    ...
    if (address >= 0x800 && address < 0xA00)                                    // only RAM space is writeable
    {
      ...
      println("invalid attempt to write memory "+hex(address));
    }
}
...
//            screenMemory = (R[0] & 0xFF00)-0x800;                        // masking with $FF00
//            #else
